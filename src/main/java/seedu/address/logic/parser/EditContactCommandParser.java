...
public class EditContactCommandParser implements Parser<EditContactCommand> {
    ...
    public EditContactCommand parse(String args) throws ParseException {
        ...
        ArgumentMultimap argMultimap =
                ArgumentTokenizer.tokenize(args, PREFIX_NAME, PREFIX_PHONE, PREFIX_EMAIL,
                        PREFIX_COMPANY, PREFIX_ADDRESS, PREFIX_TAG, PREFIX_COMPANY_ROLE);
        ...
        parseCompanyRolesForEdit(argMultimap.getAllValues(PREFIX_COMPANY_ROLE))
                .ifPresent(editPersonDescriptor::setCompanyRoles);
    ...
    private Optional<Set<CompanyRole>> parseCompanyRolesForEdit(Collection<String> companyRoles) throws ParseException {
        assert companyRoles != null;

        if (companyRoles.isEmpty()) {
            return Optional.empty();
        }
        Collection<String> companyRoleSet = companyRoles.size() == 1
                && companyRoles.contains("") ? Collections.emptySet() : companyRoles;
        return Optional.of(ParserUtil.parseCompanyRoles(companyRoleSet));
    }
